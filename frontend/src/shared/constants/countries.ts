...
const callingCodes: Record<string, string> = {
  ...
  GY: '+592', HK: '+852', HN: '+504', HR: '+385', HT: '+509', HU: '+36', ID: '+62', IE: '+353', IM: '+44',
...
export const countries: CountryOption[] = Object.entries(callingCodes)
  .filter(([iso]) => iso !== 'IL')
  .map(([iso, callingCode]) => ({
    iso,
    callingCode,
...
export const getCountryName = (iso: string, language: string) => {
  if (iso === 'PS') return language.startsWith('ar') ? 'فلسطين' : language.startsWith('fr') ? 'Palestine' : 'Palestine';
  try {
...
export const getCountryByIso = (iso: string) => countries.find((country) => country.iso === iso);
